<?php
	...
	$tblTenantJournals = new Table("TenantJournals", "journal_", array
	(
		// 			$name, $dataType, $size, $value, $allowNull, $primaryKey, $autoIncrement
		new Column("ID", "INT", null, null, false, true, true),
		new Column("TenantID", "INT", null, null, false),
	...
	$tblTenantJournals->ForeignKeys = array
	(
		new TableForeignKey("TenantID", new TableForeignKeyColumn($tblTenants, "ID")),
		new TableForeignKey("CreationUserID", new TableForeignKeyColumn($tblUsers, "ID"))
	);
	$tables[] = $tblTenantJournals;
	
	// Object static property journal entries.
	$tblTenantPropertyJournalEntries = new Table("TenantPropertyJournalEntries", "entry_", array
	(
		// 			$name, $dataType, $size, $value, $allowNull, $primaryKey, $autoIncrement
	...
	$tblTenantPropertyJournalEntries->ForeignKeys = array
	(
		new TableForeignKey("JournalID", new TableForeignKeyColumn($tblTenantJournals, "ID")),
		new TableForeignKey("PropertyID", new TableForeignKeyColumn($tblTenantProperties, "ID"))
	);
	$tables[] = $tblTenantPropertyJournalEntries;
?>