<?php
	...
	(
		...
		new Record(array
		(
			new RecordColumn("Name", "IPAddress"),
			new RecordColumn("Description", "Stores four groups of single-byte (0-255) values representing an Internet Protocol v4 (IPv4) address."),
			new RecordColumn("EncoderCodeBlob", ColumnValue::Undefined),
			new RecordColumn("DecoderCodeBlob", ColumnValue::Undefined),
			new RecordColumn("ColumnRendererCodeBlob", ColumnValue::Undefined),
			new RecordColumn("EditorRendererCodeBlob", ColumnValue::Undefined)
...
if (!isset($name))
{
	echo("<!-- No field name specified for column renderer -->");
	$name = "";
}
...
if ($inst != null)
{
	...
	echo("t.onkeyup = function(e) { ");
	echo("var searchURL = \"" . PhoenixSNS::BuildSearchURL(array($input->ValidObjects[0]->DefaultProperty->ID => "%query%"), $input->ValidObjects) . "\";");
	echo("searchURL = searchURL.replace('%query%', t.value);");
	echo("var xhr = new XMLHttpRequest();");
	echo("xhr.onreadystatechange = function() { ");
	echo("if (xhr.readyState == 4) { alert (xhr.responseText); }");
	echo(" };");
	echo("xhr.open('GET', searchURL, true);");
	echo("xhr.send(null);");
	echo("};");
	echo("</script>");
echo("</div>");
...
{
	$bt = debug_backtrace();
	$msg = "Expected MultipleInstanceProperty, got something else";
	if (count($bt) > 1)
	{
		$msg .= " in " . $bt[1]["file"] . "::" . $bt[1]["function"] . " at line " . $bt[1]["line"];
	}
	else if (count($bt) > 0)
	{
		$msg .= " in " . $bt[1]["file"] . "::" . $bt[1]["function"] . " at line " . $bt[1]["line"];
