<?php
	...
	$objUserAgent = $tenant->CreateObject("UserAgent",
	array
	(
		new TenantStringTableEntry($langEnglish, "User Agent")
	),
	array
	(
		new TenantStringTableEntry($langEnglish, "A User Agent that can determine what browser or client the guest is using to access the Web site.")
	),
	array
	(
	...
	$objUserAgentBehavior = $tenant->CreateObject("UserAgentBehavior",
	array
	(
		new TenantStringTableEntry($langEnglish, "User Agent Behavior")
	),
	array
	(
		new TenantStringTableEntry($langEnglish, "The behavior to use when determining whether a list of user agents is allowed (whitelist) or denied (blacklist).")
	),
	array
	(
		new TenantObjectInstanceProperty("Name")
	...
	$objPageSection = $tenant->CreateObject("PageSection",
	array
	(
		new TenantStringTableEntry($langEnglish, "Page Section")
	),
	array
	(
		new TenantStringTableEntry($langEnglish, "Represents a section of a Page on the tenant. Sections can be assigned to one or more Pages, and can be duplicated within the same Page.")
	),
	array
	(
		new TenantObjectInstanceProperty("Name"),
	...
	$object = $tenant->CreateObject("Page",
	array
	(
		new TenantStringTableEntry($langEnglish, "Page")
	),
	array
	(
		new TenantStringTableEntry($langEnglish, "Represents an individual Page on the tenant. Pages have URLs (such as ~/dashboard or ~/account/login) and host one or more PageSections.")
	),
	array
	(
	...
	(
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Name"), "GuestMainPageDashboardPanel"),
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Content"), <<<'EOD'
<?php
...
<div class="Card">
	...
	<div class="Content">
		If you are the site administrator, please make sure you have configured your PhoenixSNS installation correctly. Log
...
EOD
	...
	(
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Name"), "GuestMainPageRegisterPanel"),
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Content"), <<<'EOD'
<?php
	use WebFX\System;
?>
<div class="Card">
	<form id="frmRegisterDefault" action="<?php echo(System::ExpandRelativePath("~/account/register.page")); ?>" method="POST" style="width: 100%;">
		...
		<div class="Content">
			...
			<div class="Form">
				<?php  
					// Fields required by PhoenixSNS (login ID, password, display name, and URL name) come first. After the user
					// provides the initial required information, you may follow up with custom PhoenixSNS sign-up pages.
				?>
...
EOD
	...
	(
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Name"), "GuestMainPageLoginPanel"),
		new TenantObjectInstancePropertyValue($objPageSection->GetInstanceProperty("Content"), <<<'EOD'
<?php
	use WebFX\System;
?>
...
EOD
	...
	(
		new TenantObjectInstancePropertyValue($object->GetInstanceProperty("Name"), "GuestMainPage"),
		new TenantObjectInstancePropertyValue($object->GetInstanceProperty("Sections"), new MultipleInstanceProperty
		(
			array
			(
